...
//add by hr 190504
//*********************************
cv::Mat get_cur_position_pub()
{
    return cur_position_for_pub;
}
/*void make_cur_pos_pub(cv::Mat make_pos)
{
    make_pos.copyTo(cur_position_for_pub);
}*/
//*********************************
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    if(!mCurrentFrame.mTcw.empty()) {
        cv::Mat aa;//(3,3,CV_32FC1,cv::Scalar::all(0.0));
        aa = mCurrentFrame.mTcw.rowRange(0, 3).colRange(0, 3).t();
        aa.copyTo(cur_position_for_pub.rowRange(0, 3).colRange(0, 3));
        for (int i = 0; i < 3; i++) {
            float x = 0;
            for (int j = 0; j < 3; j++) {
                x = x + aa.at<float>(i, j) * mCurrentFrame.mTcw.at<float>(j, 3);
            }
            cur_position_for_pub.at<float>(i, 3) = -x;
        }
        //cv::Mat bb = -aa*(mCurrentFrame.mTcw.col(3));
        //bb.copyTo(cur_position_for_pub.col(3));
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK)
        {
            ...
            //add by hr 190504
            //***********************************************************
            //cur_position_for_pub = mCurrentFrame.mTcw;
            //***********************************************************
